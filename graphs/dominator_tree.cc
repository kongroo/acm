...
vector<vector<int>> get_dominator_tree(const vector<vector<int>> &G, int root) {
    ...
    vector<int> In(n, -1), S(n), Idom(n), Semi(n), B(n), Fa(n), Pre(n);
    vector<vector<int>> Dom(n), rG(n), R(n);
    for (int u = 0; u < n; u++) for (auto v : G[u]) rG[v].push_back(u);
    for (int u = 0; u < n; u++) Fa[u] = B[u] = Semi[u] = u;
    ...
    auto dfs = [&](int u, auto f) -> void {
        In[u] = sz, S[sz++] = u;
        for (auto v : G[u]) if (!~In[v]) f(v, f), Pre[In[v]] = In[u];
    };
    dfs(root, dfs);
    for (int i = sz - 1, u, v; u = S[i], i > 0; --i) {
        for (auto rv : rG[u]) if (~(v = In[rv]))
                get(v, get), Semi[i] = min(Semi[i], Semi[B[v]]);
        Dom[Semi[i]].push_back(i);
        int x = Fa[i] = Pre[i];
        for (auto z : Dom[x])
            get(z, get), Idom[z] = Semi[B[z]] < x ? B[z] : x;
        Dom[x].clear();
    }
    for (int i = 1; i < sz; i++) {
    ...
    for (int u = 0; u < n; u++) for (auto v : Dom[u]) R[S[u]].push_back(S[v]);
    return R;
}
