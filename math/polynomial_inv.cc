...
template <typename T>
vector<T> multiply(const vector<T>& A, const vector<T>& B, int p = 1e9 + 7) {
...
int mpow(long long a, int k, int m) {
  int r(1);
...
template <typename T>
vector<T> inv(const vector<T>& A, int n, int p) {
