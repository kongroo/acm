...
struct DLX {
  ...
  DLX(int n, int m, vector<pair<int, int>> P) : n(n), m(m), sz(0), S(m + 1) {
    ...
    for (auto p : P) {
      ...
      if (!F[r])
        add(sz, C[c], sz, c, r, c), F[r] = sz;
      else
        add(F[r], C[c], sz - 1, c, r, c);
      L[R[sz]] = R[L[sz]] = U[D[sz]] = D[U[sz]] = sz, C[c] = sz++, S[c]++;
    }
  }
  void add(int r, int u, int l, int d, int i, int j) {
    R.push_back(r), U.push_back(u), L.push_back(l), D.push_back(d), I.push_back(i), J.push_back(j);
  }
  void remove(int c) {
    L[R[c]] = L[c], R[L[c]] = R[c];
    for (int i = D[c]; i != c; i = D[i])
      for (int j = R[i]; j != i; j = R[j]) U[D[j]] = U[j], D[U[j]] = D[j], --S[J[j]];
  }
  void restore(int c) {
    for (int i = U[c]; i != c; i = U[i])
      for (int j = L[i]; j != i; j = L[j]) U[D[j]] = D[U[j]] = j, ++S[J[j]];
    L[R[c]] = R[L[c]] = c;
  }
...
int main() {
  for (int i = 1; i <= 9; i++)
    for (int j = 1; j <= 9; j++) scanf("%d", &G[i][j]);
  vector<pair<int, int>> P;
  auto row_col = [](int i, int k) { return 81 + (i - 1) * 9 + k; };
  auto col_col = [](int j, int k) { return 81 * 2 + (j - 1) * 9 + k; };
  auto block_col = [](int i, int j, int k) {
    return 81 * 3 + ((i - 1) / 3 * 3 + (j - 1) / 3) * 9 + k;
  };
  vector<int> Used;
  int cnt = 0;
  for (int i = 1; i <= 9; i++) {
    for (int j = 1; j <= 9; j++) {
      ...
      for (int k = 1; k <= 9; k++) {
        ...
        if (k == G[i][j])
          for (auto x : {a, b, c, d}) Used.push_back(x);
      }
    }
  }
  ...
  if (Ans.size() != 1u)
    ...
    for (int i = 1; i <= 9; i++)
      for (int j = 1; j <= 9; j++) printf("%d%c", G[i][j], " \n"[j == 9]);
  }
  return 0;
}
