...
struct SinglePST {
  ...
  SinglePST(const vector<T>& A, T unit = T(), Func op = plus<T>(), Func al = plus<T>())
    ...
    for (int i = n - 1; i; i--) D[i] = {op(D[i * 2].v, D[i * 2 + 1].v), i * 2, i * 2 + 1};
  }
  ...
  void modify(int p, T val, int t = -1) {
    static const auto modify_ = [&](int o, int a, int b, auto f) -> int {
      ...
      if (a == M)
        ...
        if (p < M)
          tmp = f(D[o].lc, a, M, f), D[no].lc = tmp;
        else
          tmp = f(D[o].rc, M, b, f), D[no].rc = tmp;
        D[no].v = op(D[D[no].lc].v, D[D[no].rc].v);
      }
      return no;
    };
    assert(0 <= p && p < n), R[get_ver(t)] = modify_(R[get_ver(t)], 0, n, modify_);
  }
  T query(int l, int r, int t = -1) {
    ...
    return assert(l < r && 0 <= l && r <= n), query_(R[get_ver(t)], 0, n, query_);
  }
};
