...
struct SuffixAutomaton {
  ...
  vector<State> D;
  template <typename X>
  SuffixAutomaton(const X& Seq) : D(2) {
    for (auto c : Seq) insert(c);
  }
  void insert(int c) {
    c -= B, assert(0 <= c && c < N);
