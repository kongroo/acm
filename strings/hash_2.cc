...
struct Hash2 {
  ...
  constexpr int add(int x, int y, int m) {
    int t = x + y;
    return t < 0 ? t + m : t < m ? t : t - m;
  }
  template <typename T>
  Hash2(const T& S, int p1 = 131, int p2 = 1313, int m1 = 1e9 + 7, int m2 = 1e9 + 9)
  ...
  pair<int, int> get(int l, int r) {
    return {add(H[0][l], -mul(H[0][r], H[2][r - l], m1), m1),
